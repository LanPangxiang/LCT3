...
"""
   Copied from https://github.com/test-time-training/ttt-lm-pytorch
"""
...
try:
...
except Exception:  # Keep base TTT layers usable even when transformers is unavailable.
    ...
    class BaseModelOutputWithPast(tuple):
        pass

    class CausalLMOutputWithPast(tuple):
        pass
    ...
    class _CompatLogging:
        ...
        def get_logger(name):
            ...
            if not hasattr(logger, "_warning_once_cache"):
                logger._warning_once_cache = set()

                def warning_once(msg, *args, **kwargs):
                    rendered = msg % args if args else msg
                    if rendered in logger._warning_once_cache:
                        return
                    logger._warning_once_cache.add(rendered)
                    logger.warning(msg, *args, **kwargs)

                logger.warning_once = warning_once
            return logger

    logging = _CompatLogging()
...
class SwiGluMLP(nn.Module):  #前馈网络 Feed Forward Network
...
class TTTBase(nn.Module):
    ...
    def get_ttt_inputs(self, inputs, mini_batch_size, cache_params, token_mask=None):
        XQ = inputs["XQ"]
        XK = inputs["XK"]
        XV = inputs["XV"]
        X = inputs["X"]
        ...
        if token_mask is not None:
            ...
            # [B, L] -> [B, 1, num_mini_batch, mini_batch_size, 1]
            token_mask = token_mask.reshape(B, num_mini_batch, mini_batch_size).unsqueeze(1).unsqueeze(-1)
            eta = eta * token_mask.to(dtype=eta.dtype)
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if token_mask.dtype != torch.bool:
                token_mask = token_mask != 0
            hidden_states = hidden_states * token_mask.to(dtype=hidden_states.dtype).unsqueeze(-1)
        ...
        if num_mini_batch > 0:
            ...
            mask_main = token_mask[:, : num_mini_batch * self.mini_batch_size] if token_mask is not None else None
            output_mod, last_mini_batch_params_dict = self.ttt(
                self.get_ttt_inputs(inputs, self.mini_batch_size, cache_params, token_mask=mask_main),
        ...
        if reminder_len > 0:
            ...
            mask_rem = token_mask[:, -reminder_len:] if token_mask is not None else None
            output_reminder, _ = self.ttt(
                self.get_ttt_inputs(inputs, reminder_len, cache_params, token_mask=mask_rem),
...
class Block(nn.Module):  #对应论文中的一个 Residual Block。
    def __init__(self, config: TTTConfig, layer_idx: int):
        ...
        self.mlp = SwiGluMLP(config)  #前馈网络 Feed Forward Network
        if self.pre_conv:
