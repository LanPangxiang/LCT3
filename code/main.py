...
gol.pLog(f'CUDA available: {torch.cuda.is_available()}')
if torch.cuda.is_available():
    gol.pLog(f'CUDA current_device: {torch.cuda.current_device()}')
    gol.pLog(f'CUDA device_name: {torch.cuda.get_device_name(0)}')
else:
    gol.pLog('CUDA not available, running on CPU.')
